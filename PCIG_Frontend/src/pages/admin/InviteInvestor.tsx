...
export default function InviteInvestor() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.post('/admin/investors/invite', formData);
            if (response.data.success) {
                navigate('/admin/investors');
            } else {
                alert('Failed to invite investor: ' + response.data.message);
            }
        } catch (error: any) {
            console.error('Error inviting investor:', error);
            alert('Error inviting investor: ' + (error.response?.data?.message || error.message));
        } finally {
                    ...
                    <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, border: '1px solid #E2E8F0', padding: isMobile ? 16 : 24 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 28 }}>
                                ...
                                />
                                <label htmlFor="isAccredited" style={{ fontSize: 13, fontWeight: 500, color: '#334155', cursor: 'pointer' }}>
                                    Accredited Investor
                                </label>
                            </div>
                            <div style={{ gridColumn: isMobile ? '1' : '1 / -1' }}>
                                ...
                                >
                                    <option value="">{isLoadingFunds ? 'Loading funds...' : '-- No fund assignment --'}</option>
