...
export default function FIFAImport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching FIFA import data:', err);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Reset mapping confirmation when new file selected
  ...
  const handleStartImport = async () => {
    ...
    // Check if mapping is confirmed, if not, ask user to confirm default mappings
    if (!mappingConfirmed) {
      const proceed = window.confirm("You haven't explicitly confirmed the field mappings in the section below. Do you want to proceed with the default mappings?");
      if (!proceed) return;
      // If user confirms, we consider mapping confirmed and proceed
    }
    ...
    try {
      ...
      alert('Failed to start import. Please check console.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleReviewAction = async (id: string, action: 'approve' | 'reject') => {
    try {
      if (action === 'approve') {
        await api.post(`/admin/fifa/review/${id}/approve`);
      } else {
        await api.post(`/admin/fifa/review/${id}/reject`);
      }
      // Refresh dashboard
      const response = await api.get('/admin/fifa/import-dashboard-data');
      setData(response.data);
          ...
          <div style={{ ...cardStyle, width: '100%', minWidth: 0 }}>
            ...
            >
              {uploadSection.uploadAreas.map((area: any) => {
                ...
                // Determine if this area matches the selected file type
                const isMatchingType = uploadFile && (
                  (area.id === 'excel' && /\.(xlsx|xls)$/i.test(uploadFile.name)) ||
                  (area.id === 'csv' && /\.csv$/i.test(uploadFile.name))
                );
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isMatchingType ? `Selected: ${uploadFile.name}` : area.buttonText}
                    </button>
                    <div
                      style={{
                        fontSize: isMobile ? 11 : 12,
            ...
            <div
              onClick={() => window.open('/fifa_import_template.xlsx', '_blank')}
              style={{
                fontSize: isMobile ? 12 : 13,
                color: '#2563EB',
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: isMobile ? 10 : 12, marginTop: isMobile ? 16 : isTablet ? 18 : 20, flexWrap: 'wrap' }}>
              {uploadSection.formActions.map((action: any, idx: number) => (
                <button
                  ...
                  disabled={action.variant === 'primary' && uploading}
                  style={{
                    padding: isMobile ? '8px 16px' : '10px 20px',
                    borderRadius: 8,
                    border: action.variant === 'primary' ? 'none' : '1px solid #E2E8F0',
                    backgroundColor: action.variant === 'primary' ? (uploading ? '#93C5FD' : '#2563EB') : '#FFFFFF',
                    color: action.variant === 'primary' ? '#FFFFFF' : '#64748B',
                    fontSize: isMobile ? 13 : 14,
                    fontWeight: 500,
                    cursor: action.variant === 'primary' && uploading ? 'not-allowed' : 'pointer',
                    whiteSpace: 'nowrap',
                    boxSizing: 'border-box'
                  }}
                >
                  {action.variant === 'primary' && uploading ? 'Importing...' : action.label}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ ...cardStyle, width: '100%', minWidth: 0 }}>
                  ...
                  {fieldMapping.mappings.map((mapping: any, idx: number) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #E2E8F0' }}>
                      ...
                      <td style={{ padding: isMobile ? '12px' : '14px 16px', color: '#64748B', wordBreak: 'break-word' }}>
                        {uploadFile ? (
                          <span style={{ color: '#10B981', fontWeight: 500 }}>Auto-matched</span>
                        ) : (
                          mapping.fileColumn
                        )}
                      </td>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: isMobile ? 16 : isTablet ? 18 : 20 }}>
              <button
                ...
                disabled={mappingConfirmed || !uploadFile}
                style={{
                  padding: isMobile ? '8px 16px' : '10px 20px',
                  borderRadius: 8,
                  border: 'none',
                  backgroundColor: mappingConfirmed ? '#10B981' : (uploadFile ? '#2563EB' : '#94A3B8'),
                  color: '#FFFFFF',
                  fontSize: isMobile ? 13 : 14,
                  fontWeight: 500,
                  cursor: mappingConfirmed || !uploadFile ? 'default' : 'pointer',
                  whiteSpace: 'nowrap',
                  boxSizing: 'border-box',
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                {mappingConfirmed && <CheckCircle2 size={16} />}
                {mappingConfirmed ? 'Mapping Confirmed' : fieldMapping.actionButton}
              </button>
            </div>
          </div>
                        ...
                        <div style={{ display: 'flex', gap: isMobile ? 6 : 8, flexWrap: 'wrap' }}>
                          {item.actions.map((action: string, idx: number) => (
                            <button
                              key={idx}
                              onClick={() => {
                                if (action === 'Confirm') handleReviewAction(item.id, 'approve');
                                if (action === 'Reject') handleReviewAction(item.id, 'reject');
                              }}
                              style={{
                                ...
                                border: '1px solid #E2E8F0',
                                backgroundColor:
                                  action === 'Confirm' ? '#10B981' : action === 'Reject' ? '#DC2626' : '#FFFFFF',
                                color:
