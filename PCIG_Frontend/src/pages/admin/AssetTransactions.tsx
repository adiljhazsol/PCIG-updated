...
interface TransactionActionButton {
  label: string;
  color: string;
  icon?: string;
}

interface TransactionActionButtons {
  terminate: TransactionActionButton;
  updateStatus: TransactionActionButton;
  closeTransaction: TransactionActionButton;
}
...
interface SelectedTransaction {
  ...
  actionButtons: TransactionActionButtons;
}
...
interface DashboardData {
  assetTransactions: {
    ...
    actionButtons: {
      export: ActionButton;
      createTransaction: ActionButton;
    };
    tableHeaders: string[];
...
export default function AssetTransactions() {
  ...
  // State for checkbox selections - Moved to top level to avoid conditional hook call
  const [selectedTransactions, setSelectedTransactions] = useState<Set<string>>(new Set());

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/admin/transactions/dashboard-data');
  ...
  // Update selected transactions when data is loaded
  useEffect(() => {
    if (data?.assetTransactions?.transactions) {
      const initialSelected = new Set(
        data.assetTransactions.transactions
          .filter((t: TransactionSummary) => t.selected)
          .map((t: TransactionSummary) => t.id)
      );
  ...
  const transactionsData = data?.assetTransactions || {};
  const header = transactionsData?.header || { title: '', subtitle: '' };
  const actionButtons = transactionsData?.actionButtons || { export: { label: 'Export', icon: 'Download' }, createTransaction: { label: 'New Transaction', icon: 'Plus' } };
  ...
  const selectedTransaction = transactionsData?.selectedTransaction || null;

  // Handle checkbox change
  const handleCheckboxChange = (transactionId: string) => {
    setSelectedTransactions((prev: Set<string>) => {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedTransactions(new Set(transactions.map((t: TransactionSummary) => t.id)));
    } else {
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 16 : 20 }}>
            ...
            >
              ...
              >
                {React.createElement(iconMap[actionButtons?.export?.icon] || Download, { style: { width: 16, height: 16, flexShrink: 0 } })}
                {actionButtons?.export?.label || 'Export'}
              </button>
              <button
                onClick={() => navigate('/admin/asset-transactions/create')}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: 6,
              ...
              >
                {React.createElement(iconMap[actionButtons?.createTransaction?.icon] || Plus, { style: { width: 16, height: 16, flexShrink: 0 } })}
                {actionButtons?.createTransaction?.label || 'New Transaction'}
              </button>
            </div>
            ...
            >
              ...
              >
                ...
                <tbody>
                  {transactions.map((transaction: TransactionSummary) => (
                    <tr
                      key={transaction.id}
                      style={{
          ...
          >
            ...
            <div style={{ marginBottom: isMobile ? 16 : 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 12 }}>
                {selectedTransaction.timeline.map((step: TimelineStep, idx: number) => (
            ...
            <div style={{ marginBottom: isMobile ? 16 : 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {selectedTransaction.documents.map((doc: Document, idx: number) => (
                  <div
                    key={idx}
                    style={{
                      display: 'flex',
            ...
            <div style={{ marginBottom: isMobile ? 16 : 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 12 }}>
                {selectedTransaction.activityLog.map((activity: ActivityLogEntry, idx: number) => (
                  <div key={idx}>
                    <div
                      style={{
                        fontSize: isMobile ? 'clamp(10px, 2.5vw, 12px)' : '12px',
            ...
            >
              ...
              >
                {selectedTransaction.actionButtons.closeTransaction.icon && React.createElement(iconMap[selectedTransaction.actionButtons.closeTransaction.icon], {
