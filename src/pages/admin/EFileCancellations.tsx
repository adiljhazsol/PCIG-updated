...
export default function EFileCancellations() {
  ...
  const efileData = adminData.efileCancellations;
  const header = efileData.header;
  const summaryCards = efileData.summaryCards;
  const alertBanners = efileData.alertBanners;
  const searchAndFilters = efileData.searchAndFilters;
  const cancellationsTable = efileData.cancellationsTable;
  const detailPanel = efileData.detailPanel;

  const [selectedCancellations, setSelectedCancellations] = useState<Set<string>>(
    new Set(cancellationsTable.rows.filter((r: any) => r.selected).map((r: any) => r.id))
  );
  const [selectedCancellationId, setSelectedCancellationId] = useState<string>(
    cancellationsTable.rows.find((r: any) => r.selected)?.id || cancellationsTable.rows[0].id
  );
  ...
  const selectedCancellation = cancellationsTable.rows.find((r: any) => r.id === selectedCancellationId) || cancellationsTable.rows[0];
  ...
  const SettingsIcon = iconMap[header.actionButtons[0].icon] || Settings;
  const BatchEFileIcon = iconMap[header.actionButtons[1].icon] || Upload;
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 16 : isTablet ? 18 : 20, minWidth: 0, width: '100%', order: isMobileOrTablet ? -1 : 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span
                    style={{
                      ...
                      fontSize: isMobile ? 10 : 11,
                      fontWeight: 500,
                      backgroundColor: selectedCancellation.statusBg,
                      color: selectedCancellation.statusColor,
                      whiteSpace: 'nowrap'
                    }}
                  >
            ...
            <div style={{ ...cardStyle, width: '100%', minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 8 : 10 }}>
                {detailPanel.cancellationDetails.fields.map((field: any, idx: number) => (
